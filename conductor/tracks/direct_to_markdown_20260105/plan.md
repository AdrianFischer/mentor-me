...
- [ ] Task: Implement `FileSystemWatcher` to listen for external changes.
...
- [ ] Task: Remove `isar` and `isar_flutter_libs` from `pubspec.yaml`.
- [ ] Task: Delete `IsarService` and all `.g.dart` files generated by Isar.
- [ ] Task: Remove Isar initialization logic from `main.dart`.
