...
*   **Shared Folder Access (Restricted Terminal):** The agent has access to a "Shared" folder (`data/shared`) for file interactions and terminal commands.
    1.  **Tools:** `list_shared_files`, `read_shared_file`, `write_shared_file`, and `run_shared_command`.
    2.  **Restriction:** `run_shared_command` executes with `cwd` set to the shared folder, and path validation prevents escaping the directory. This allows the Telegram bot to perform development or data tasks in a sandboxed environment.
...
*   **Decoupling MCP (Node.js Autonomy):** The Node.js agent should not rely on the Flutter app (via MCP on port 8081) for reading/writing core data (Projects/Todos). Relying on the Flutter app causes `ECONNREFUSED` crashes when the app is closed. Task logic must be ported to native Node.js tools using `data_service.js` to ensure 24/7 offline autonomy for the Telegram bot and background Watchdog routines. A migration plan is tracked at `docs/plans/PLAN_DECOUPLE_MCP.md`.
*   **Docker Container Updates (CRITICAL):** When modifying Node.js code (e.g. in `agent/`), you MUST rebuild the Docker image (`docker-compose build` or `make build`) before restarting the services (`make down && make start`). Restarting without building will run the stale, broken code and cause the same bug twice.
*   **Zero-Repetition Mistake Policy (MANDATORY):** We operate on a strict "make a mistake only once" policy. If an error occurs (e.g., a bug in deployment, an incorrect command sequence, or an architectural misunderstanding), the root cause and the specific resolution steps MUST be immediately documented in `docs/GEMINI.md` under 'Learnings & Future Context'. Future agent sessions must read and apply these learnings proactively to ensure the same error never happens again.
